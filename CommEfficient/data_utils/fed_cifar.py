...
__all__ = ["FedCIFAR10", "FedCIFAR10Pretrained", "FedCIFAR100", "FedFashionMNIST"]
...
class FedCIFAR10Pretrained(FedCIFAR10):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)

    def prepare_datasets(self, download=True):
        os.makedirs(self.dataset_dir, exist_ok=True)
        dataset = torchvision.datasets.__dict__.get("CIFAR10")
        vanilla_train = dataset(self.dataset_dir,
                                train=True,
                                download=download)
        vanilla_test = dataset(self.dataset_dir,
        ...
        feature_path = self.dataset_dir + "features/cifar100_resnext"
        train_images = np.load(f"{feature_path}_train.npy")
        test_images = np.load(f"{feature_path}_test.npy")
        #train_images = vanilla_train.data
        train_targets = np.array(vanilla_train.targets)
        classes = vanilla_train.classes
