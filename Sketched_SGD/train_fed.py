...
def train(worker, epochs=24, batch_size, loggers=(), timer=None):
    ...
    for epoch in range(epochs):
        train_loss, train_acc = ray.get(worker.train_epoch.remote(True))
        train_time = timer()
        test_loss, test_acc = ray.get(worker.train_epoch.remote(False))
        test_time = timer()
        stats = {
...
if __name__ == "__main__":
    ...
    # train(ps, workers, args.epochs, args.epochs_per_iter, kwargs, args.epochs,
